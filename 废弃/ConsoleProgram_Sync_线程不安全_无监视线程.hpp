...
public:
	...
	默认工作目录为文件所在目录
	*/
	ConsoleProgram_Sync(const std::string& programPath,
	                    const std::string& workingDirectory = "",
	                    const std::string& commandLineArgument = "")
		: programPath(programPath), workingDirectory(workingDirectory),
		  commandLineArgument(commandLineArgument), processExitCode(0),
		  inputPipeRead(NULL), inputPipeWrite(NULL),
		  outputPipeRead(NULL), outputPipeWrite(NULL), processHandle(NULL) {
		//初始化指针
	...
	bool Start() {
		...
		//创建进程
		if (!CreateProcess(NULL, commandLine_c, NULL, NULL, TRUE,
		                   CREATE_NO_WINDOW, NULL, workingDirectory.c_str(),
		...
		if (!getProcessStatus()) {
			if (lastOutputBuffer != NULL) {
				...
				if (lastOutputBufferLen < bufferSize) {
					...
					//复制到新空间，并且释放旧的
					strncpy(tmp, lastOutputBuffer + bufferSize - 1,
					        lastOutputBufferLen);
